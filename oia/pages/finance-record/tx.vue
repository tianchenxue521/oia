<template>
	<view class="root">
		<!-- <u-navbar title="提现记录" :placeholder="true" :autoBack="true" bgColor="#F3F3F3" :titleStyle="{color:'#333',fontWeight: 'bold'}"></u-navbar> -->
		<u-navbar title="提现记录" :placeholder="true" :autoBack="true"
		bgColor="#222" 
		...
		leftIconColor="#FFF">
			...
			<view class="item ddds" v-for="(item,index) in dataList" :key="index">
				<view class=" dfb" >
					<view class="left">
						<view class="left_cz_text">
							<text class="left_cz_text_s1">提现</text>
							<text v-if="item.paytype == 5">
					...
					<view class="right">
						...
						<view v-if="item.status==1">￥{{item.amount}}</view>
				...
				<view style="font-size: 24rpx;color: #999;margin-top: 16rpx;" v-if="item.remarks!==''&&item.remarks!==null&&item.status==2">失败原因：{{item.remarks}}</view>
			</view>
		</z-paging>
	</view>
</template>
